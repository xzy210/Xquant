...
EVIDENCE_ROOT = Path(__file__).resolve().parents[2] / "agent_evidence"
...
class AgentEvidenceService:
    ...
    def cleanup_old_artifacts(self) -> None:
        """Trim old evidence reports and temporary images."""
        try:
    ...
    def _cleanup_evidence_runs(self) -> None:
        ...
        for run_file in self.runs_dir.glob("*.md"):
            if run_file not in indexed_paths:
                created_at = self._safe_mtime(run_file)
    ...
    def _cleanup_temp_images(self) -> None:
        ...
        patterns = [
            f"{TEMP_KLINE_PREFIX}*.png",
            f"{TEMP_PASTED_PREFIX}*.png",
        ]
